...
def generate_dadda_multiplier():
    ...
    # Each list holds the exact string identifiers for wires at that binary weight.
    ...
    for stage_idx, limit in enumerate(dadda_limits):
        ...
        for c in range(256):
            # Compress the current column until its height meets the stage limit
            while len(columns[c]) > limit:
                # If exactly 1 bit over limit, use a Half Adder
                if len(columns[c]) == limit + 1:
                    in1 = columns[c].pop()
                    in2 = columns[c].pop()
                    ...
                    instantiations.append(f"    ha ha_{stage_idx}_{ha_count} (.a({in1}), .b({in2}), .sum({sum_wire}), .carry({carry_wire}));")
                ...
                # If 2 or more bits over limit, use a Full Adder
                    ...
                    wire_declarations.append(f"logic {sum_wire}, {carry_wire};")
                    instantiations.append(f"    fa fa_{stage_idx}_{fa_count} (.a({in1}), .b({in2}), .c({in3}), .sum({sum_wire}), .carry({carry_wire}));")
...
""")
        ...
        # Dadda Tree Logic Instantiations
        fh.write("    // Dadda Tree Adder Instantiations\n")
        fh.write("\n".join(instantiations) + "\n\n")
